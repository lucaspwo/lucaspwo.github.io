...
3. Edite o conteúdo (`<h2>`, parágrafos, tags `<img>` manuais — aqui é HTML puro, não há sistema de macros/templating de gerador de site estático). ⚠️ Justamente por isso, evite escrever literalmente algo parecido com sintaxe Liquid (chaves + `%`) neste ou em qualquer `.md` do repo — mesmo sem Jekyll configurado, o GitHub Pages roda Jekyll por baixo a menos que `.nojekyll` esteja presente, e um trecho assim já derrubou o deploy inteiro por 2 meses sem avisar (ver `Gotchas` e `Build / Test / Lint / Deploy` abaixo).
...
- **Deploy:** automático via GitHub Pages. **Confirmado ao vivo via `gh api repos/lucaspwo/lucaspwo.github.io/pages`**: `source.branch = "fallback"`, `source.path = "/"`, `build_type = "legacy"`, domínio customizado `lucaspwo.com` (HTTPS forçado). ⚠️ **`build_type: "legacy"` NÃO significa "sem build" — o Pages roda Jekyll mesmo assim**, e se o Jekyll falhar o deploy inteiro é abortado (a Action `pages-build-deployment` reporta `failure`, e o site continua servindo o último build que passou). **Isso já aconteceu de verdade:** o commit `93e5bd0` (19/07) introduziu `MANUTENCAO.md:61` com o texto ilustrativo `` {% image %} `` dentro de um code span — o Liquid do Jekyll lê isso como uma tag desconhecida e quebra o build — e **o site ficou travado no snapshot de 12/07 por mais de dois meses**, sem publicar nenhum push seguinte, só descoberto em 24/09 quando um post novo deu 404 em produção. **Corrigido em 24/09/2026 adicionando `.nojekyll`** na raiz (ver Gotchas abaixo) — agora o Jekyll nem roda, e **qualquer commit dado push para `fallback` vai direto para produção**, sem CI, sem revisão automática — o HTML commitado é literalmente o HTML servido. **Sempre conferir `gh run list --repo lucaspwo/lucaspwo.github.io --branch fallback --limit 3`** depois de um push para confirmar `success`, não presumir que "sem CI" significa "sem como falhar".
...
- **⚠️ `.nojekyll` na raiz desde 24/09/2026 — não remover.** Sem ele, o Jekyll do GitHub Pages processa todo `.md`/`.html` do repo (inclusive `README.md`/`CLAUDE.md`/`MANUTENCAO.md`/`AGENTS.md`, que viram páginas públicas via `jekyll-readme-index`/`jekyll-default-layout`) e qualquer erro de sintaxe Liquid em qualquer arquivo do repo — como o `{% image %}` que ficou 2 meses quebrando o build — derruba o deploy inteiro, silenciosamente (o site continua no ar, só que travado no snapshot antigo). Ver seção Deploy acima.
